...
try:
    ...
    while True:
        ...
        if image_processing:
            ...
            a = []
            a.append(LatC_module.stanley(waypoints, target_speed))
            gas, brake = LongC_module.control(speed, target_speed)


            #Plot detected lanes
            print("\nControl " + str(["{:+0.2f}".format(x) for x in a]))
            print("\ngas " + str(["{:+0.2f}".format(gas)]))
            print("\nbrake " + str(["{:+0.2f}".format(brake)]))
            ...
            LD_module.plot_state_lane(camera_array, steps, fig2, waypoints=waypoints, target_speed=target_speed)
